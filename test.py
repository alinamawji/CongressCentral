...
import sys
import unittest
from models import db, Committees, Members, Legislation
...
class DBTestCases(unittest.TestCase):
    ...
    def test_source_insert_1(self):
        ...
        r = db.session.query(Committees).filter_by(name = "committee1").one()
        self.assertEqual(str(r.name), "committee1")

        db.session.query(Committees).filter_by(name = "committee1").delete()
        db.session.commit()
    ...
    def test_source_insert_2(self):
        ...
        r = db.session.query(Members).filter_by(id = 987).one()
        self.assertEqual(str(r.id), 987)

        db.session.query(Members).filter_by(id = 987).delete()
        db.session.commit()
